...
class DrawInformation:
    ...
    def __init__(self, width, height, lst):
        ...
        pygame.display.set_caption("Vizualizer for Sorting Algorithms")
        self.set_list(lst)
...
def draw(draw_info, algo_name, ascending):
    ...
    title = draw_info.LARGE_FONT.render(f"{algo_name} - {'Ascending' if ascending else 'Descending'}", 1, draw_info.LIGHT) 
    draw_info.window.blit(title, (draw_info.width/2 - title.get_width()/2 , 5))

    controls = draw_info.FONT.render("R - Reset | SPACE - Start Sorting | A - Ascending | D - Descending", 1, draw_info.GREEN)
...
def generate_initial_list(n, min_val, max_val):
    lst=[]
...
def main():
    ...
    lst = generate_initial_list(n, min_val, max_val)
